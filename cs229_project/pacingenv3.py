...
class PacingEnv():
    ...
    def __init__(self, dt = 5, dailyBudget = 100, ctrThres = 0, maxBid = 200, cpcGoal = 100, now = 0, index = 0, maxctr = 0.02, basecost = 10.0, baseIntensity = 5.0):
        ...
        curve = np.zeros(288)
        for i in range(288):
            curve[287-i] = i/288.0 + np.sin(2*np.pi*i/288.0)
    ...
    def getReward(self,pacing,spendRatio,timeofday):
        ...
        """
        #print "type of factor:" + str(type(self.prevpacing))
        smooth = abs(self.prevpacing - self.pacing)
        #smooth = abs(self.pacingcache[-1] - self.pacing)*5.0 if len(self.pacingcache)>3 else 0.0
        #time sensitive cost
        factor = 100*(timeofday/288.0)**2
        #factor = 1
        if timeofday>=280:
            factor = 1.0 + factor*10
        #print "internal spend is:" +  str(spendRatio) + "--internal curve is:"+str(self.spendingCurve[timeofday]) + "-time is: " + str(timeofday)
        #return -factor*(self.dailyBudget-self.dailyBudget*spendRatio - self.dailyBudget*self.spendingCurve[timeofday])**2-smooth
        #print "type of factor:" + str(type(smooth))
        #return -factor*(spendRatio - self.spendingCurve[timeofday])**2-10*smooth
        return -abs(spendRatio - self.spendingCurve[timeofday])
    
    def getRewardPrint(self,pacing,spendRatio,timeofday):
        #no penalty range is +/- thres
        """thres = 0.00001
        terminal_factor = 1000.0
        #TODO/ add cost for signal discontinuity
        ...
        #print "internal spend is:" +  str(spendRatio) + "--internal curve is:"+str(self.spendingCurve[timeofday]) + "-time is: " + str(timeofday)
        #return -factor*(self.dailyBudget-self.dailyBudget*spendRatio - self.dailyBudget*self.spendingCurve[timeofday])**2-smooth
        return -abs(spendRatio - self.spendingCurve[timeofday]), spendRatio, self.spendingCurve[timeofday], str(timeofday)
    ...
    def step(self,pacing):
        ...
        self.pacing = pacing
        self.pacingcache.append(self.pacing)
        #sratio = pacing,self.state[0]/float(self.dailyBudget)
        #tofd = self.state[1]
        #print "ratio--" + str(sratio)
        #print "tod--" + str(tofd)
        #print "cost--" + str((sratio - self.spendingCurve[tofd])**2)
        #reward = self.getRewardPrint(pacing,self.state[0]/float(self.dailyBudget),self.state[1])
        #reward,val1,val2,val3 = self.getReward(pacing,self.state[0]/float(self.dailyBudget),self.state[1])
        reward = self.getReward(pacing,self.state[0]/float(self.dailyBudget),self.state[1])
        isDone = False
        if time_left<=0:
            isDone = True
        #self.state = self.state.reshape([1,2])
        return self.state, reward, isDone, {}
    
    def stepPrint(self,pacing):
        #only simulate for one day
        if self.now >= 288*self.dt:
            return None, None, True, {}
        ...
        if self.state[0]<=0:
            ...
            self.pacing = pacing
            self.pacingcache.append(self.pacing)
            reward,val1,val2,val3 = self.getRewardPrint(pacing,self.state[0]/float(self.dailyBudget),self.state[1])
            return self.state, reward, False, {}, val1,val2,val3
        #fetch the impressions within step
        imps = []
        #imp[0]~imp[3], timestamp, click, pctr, cost
        while self.index<len(self.data):
        ...
        if time_left<=0:
            isDone = True
        #self.state = self.state.reshape([1,2])
        return self.state, reward, isDone, {},val1,val2,val3
    ...
    def resetFixdata(self):
        #won't regenerate data
        self.state = np.array([self.dailyBudget,0])
        self.now = 0L
        self.index = 0
        self.impCnt = 0
